...
export default function Programmers() {
  ...
  const [listOfProgs, setListOfProgs] = useState(listOfAwesome)
  const [featuredProgs, setFeaturedProgs] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    const id = featuredProgs - 1    
    return listOfProgs[id].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredProgs ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfProgs.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeaturedProgs(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
